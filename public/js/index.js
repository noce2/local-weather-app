$(document).ready(function() {
  ...
  function changeUnit(target) {
    if ($(target).length !== 0 /*this will go to the other branch if the target ya no existe*/ ) {
      ...
      var tempText = ($(target).text()).split(" "); //this line splits the temperature into the symbol and number
      var justNumber = Number(tempText[0]); //converts the number, which is currently a string to type int
  ...
  function getWeatherAndLocation(locNode, descripNode, tempNode, chgBtnNode, iconNode) {
    var apiTarget = "https://stormy-fortress-66721.herokuapp.com/dondeestoy";
  ...
  function getWeather(whereToLook, locNode, descripNode, tempNode, chgBtnNode, iconNode) {
    var apiTarget = "http://api.openweathermap.org/data/2.5/weather";
    var queryParams = whereToLook;
    ...
    $.getJSON(apiTarget, queryParams, function(data, textStatus, jqXHR) {
      ...
      .done(function(data) {
        ...
        var backgroundURL = giveBkgrndUrlImgProp(objOfHTML.weatherMain);
  ...
  function jsonWeatherParser(data) {  //Want to return an object with just temp and 
    ...
    var rootKeysIWant = ["weather", "main", "sys", "name"];
    var outObj = nestedFilter(data, rootKeysIWant);   //console.log(outObj);    
      
    return outObj;
  ...
  function nestedFilter(inputObj, firstLevelKeys /*,secondLevelKeys*/ ) {
    var firstFiltered = filterMyObj(inputObj, firstLevelKeys);
    //secondlevel ;
  ...
  function secondLevel(inputObj, arrOfDesiredKeys) {
    var result = {};
    for (var each in inputObj) {
      var temp = {};
      for (var subEach in inputObj[each]) {
        console.log(subEach);
  ...
  function htmlConv(toConvert) {
    var objToReturn = {};
    var temperature = String(myOneDecimalRnding(toConvert.main.temp)) + " &deg;C";
    var temperatureNode = '<p id="temperature" class="weatherInfo h1 tempAndButton">' + temperature + '</p>';
    var cityAndCountry = '<h2 id="cityAndCountry" class="weatherInfo">' + toConvert.name + ', ' + toConvert.sys.country + '</h2>';
    var weatherDescription = '<h3 id="weatherDescription" class="weatherInfo">' + capitaliseEveryFirstLetter(toConvert.weather[0].description) + '</h3>';
    var iconURL = "http://openweathermap.org/img/w/" + String(toConvert.weather[0].icon) + ".png";
    var weatherIcon = '<img src=' + iconURL + ' alt="an Icon of the current weather estara aqui"  id="weatherIcon" class="weatherInfo img-responsive center-block"/>';  
    var changeUnitsButton = '<div class="outerToggle"><input type="checkbox" checked data-toggle="toggle" data-on="&deg;C" data-off="&deg;F" data-onstyle="primary" data-offstyle="info" data-style="android" id="changeUnits" data-size="large" ></div>';
